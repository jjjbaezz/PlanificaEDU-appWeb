<template>
  <Sidebar>
    <div class="p-6 md:p-8 max-w-4xl mx-auto">
      <!-- Header -->
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Editar Información de Joselui la Cabra</h1>
        <p class="text-gray-600 mt-2">Actualiza los datos y preferencias del estudiante.</p>
      </div>
      ...
      <div class="bg-white rounded-lg shadow p-6 md:p-8">
        ...
        <div class="mb-8 pb-8 border-b border-gray-200">
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
              <span :class="['text-sm font-medium', estudiante.activo ? 'text-gray-500' : 'text-red-600']">
                Inactivo
              </span>
              <button
                @click="estudiante.activo = !estudiante.activo"
                :class="[
                  'relative inline-flex items-center h-8 w-14 rounded-full transition-colors',
                  estudiante.activo ? 'bg-blue-500' : 'bg-gray-300'
                ]"
              >
                <span
                  :class="[
                    'inline-block h-7 w-7 transform rounded-full bg-white transition-transform',
                    estudiante.activo ? 'translate-x-7' : 'translate-x-0'
                  ]"
                />
              </button>
              <span :class="['text-sm font-medium', estudiante.activo ? 'text-blue-600' : 'text-gray-500']">
                Activo
              </span>
            </div>
          </div>
        ...
        <div>
          ...
          <p class="text-gray-600 text-sm mb-6">Selecciona los bloques horarios en los que prefieres tener clases.</p>
